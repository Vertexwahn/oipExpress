...
namespace OpenInfraPlatform
{
    namespace Ifc2x3
    {
        ...
        void IfcStepReader::readStreamHeader( std::string& read_in, shared_ptr<Ifc2x3Model> model )
        {
            ...
            if( file_schema_begin != std::string::npos )
            {
                ...
                if( file_schema_end != std::string::npos )
                {
                    ...
                    {
                        m_ifc_version = Ifc2x3Model::Ifc4x1;
                    }
                    else
                    {
		...
		shared_ptr<Ifc2x3Model> IfcStepReader::read(const std::string& filename)
		{
			shared_ptr<Ifc2x3Model> m_ifc_model(new Ifc2x3Model());
			...
			readStreamHeader(buffer, m_ifc_model);
			std::string file_schema_version = m_ifc_model->getFileSchema();
			std::map<int, shared_ptr<Ifc2x3Entity> > map_entities;

			try
			{
				readStreamData(buffer, map_entities);
			}
			...
			{
				m_ifc_model->insertEntity(it->second);
			}
